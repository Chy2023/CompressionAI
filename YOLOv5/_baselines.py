...
from utils.utils import save_one_txt,save_one_json,process_batch,load_yolo
...
def run(
    ...
    if training:  # called by train.py
        ...
        stride, pt, jit, engine = max(int(yolo.stride.max()), 32),True,False,False
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[1]:
            ...
            bpp+=compute_bpp(outputs)
            x_hat=outputs['x_hat']
            preds, train_out = yolo(x=x_hat,cut_model=0) if compute_loss else (yolo(x=x_hat,cut_model=0, augment=augment), None)
            """ F=outputs['feature']
            preds, train_out = model.yolo(x=im,F=F,cut_model=2) if compute_loss else (model.yolo(x=im,F=F,cut_model=2, augment=augment), None) """
...
def main(opt):
    ...
    if opt.task in ("train", "val", "test"):  # run normally
        ...
        for model_type in model_list:
            for i in range(1,7):
                run(**vars(opt),model_type=model_type,i=i)
